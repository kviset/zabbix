#!/usr/bin/perl -w
#Description:	Script for backup redmine upload directory.
...
my $PRESTR = "script.redmine.backup.";
my @DATA = ();
my @BACKUPPATH = ("files");
########### MAIN ########### 
my $SUCCESS = 1;
...
zs_debug ("redmine Backup start time: $TIME",1,{%PARAM});
...
#Make backup
zs_debug ("Backup config to '$HOME/$PATH'",1,{%PARAM});
$RESULT = zs_system("cp -r $PROJPATH/config $HOME/$PATH",{%PARAM});
unless(defined $RESULT){ $SUCCESS = 0; }

foreach $BACKUP(@BACKUPPATH){
	if (-d "$PROJPATH/$BACKUP"){
		zs_debug ("Backuping '$PROJPATH/$BACKUP' to '$HOME/$PATH'",1,{%PARAM});
		$RESULT = zs_system("rsync -rlptD --stats $PROJPATH/$BACKUP $HOME/$PATH",{%PARAM});
